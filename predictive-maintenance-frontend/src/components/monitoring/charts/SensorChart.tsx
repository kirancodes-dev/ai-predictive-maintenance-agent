...
const SensorTooltip = ({
  ...
  return (
    <div style={{
      ...
      <div style={{ color: '#888', marginBottom: 4, fontSize: 11 }}>{label}</div>
  ...
  return (
    <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', fontSize: 12, color: '#666' }}>
      <span>
        ...
        <strong style={{ color, fontSize: 16 }}>{stats.latest.toFixed(2)} {unit}</strong>
      </span>
      {deviation !== null && (
        <span style={{ color: parseFloat(deviation) > 5 ? '#ef4444' : parseFloat(deviation) > 0 ? '#f59e0b' : '#10b981', fontWeight: 600 }}>
          {parseFloat(deviation) >= 0 ? '+' : ''}{deviation}% vs μ
        </span>
      )}
      ...
      {stats.anomalies > 0 && (
        <span style={{ background: '#fee2e2', color: '#ef4444', borderRadius: 12, padding: '1px 8px', fontWeight: 600 }}>
    ...
    <>
      ...
      {thresholds && (
        <>
          ...
          <ReferenceLine y={thresholds.criticalMax} stroke="#ef4444" strokeDasharray="4 3" strokeWidth={1.5}
          ...
          <ReferenceLine y={thresholds.criticalMin} stroke="#ef4444" strokeDasharray="4 3" strokeWidth={1.5} />
        </>
      )}
    </>
  );
      ...
      {mode === 'live' ? (
        <LineChart data={displayData} margin={{ top: 8, right: 16, left: 0, bottom: 4 }}>
          ...
          <Line type="monotone" dataKey="value" stroke={color} strokeWidth={2}
            dot={<AnomalyDot />} activeDot={{ r: 5, stroke: color, strokeWidth: 2, fill: '#fff' }}
            isAnimationActive={false} name={label} />
        </LineChart>
      ) : (
        <AreaChart data={displayData} margin={{ top: 8, right: 16, left: 0, bottom: showBrush ? 24 : 4 }}>
          <GradientDef id={gradientId} color={color} />
          {shared}
          <Area type="monotone" dataKey="value" stroke={color} strokeWidth={2}
            fill={`url(#${gradientId})`} dot={false}
            activeDot={{ r: 5, stroke: color, strokeWidth: 2, fill: '#fff' }} name={label} />
          {showBrush && (
            <Brush dataKey="timestamp" height={20} travellerWidth={6}
              tickFormatter={tickFmt} stroke={color} fill="#f8fafc" />
          )}
        </AreaChart>
      )}
    </ResponsiveContainer>
