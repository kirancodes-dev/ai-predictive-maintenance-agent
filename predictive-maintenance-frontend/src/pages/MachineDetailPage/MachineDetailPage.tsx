...
const MachineDetailPage: React.FC = () => {
  ...
  if (machineLoading) {
    return <div style={{ padding: '2rem', color: '#888' }}>Loading machine details...</div>;
  }
  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto' }}>
      ...
      {pred && (
        <div style={{ background: 'var(--color-surface, #fff)', borderRadius: 10, padding: '1.25rem', marginBottom: '1.5rem', border: '1px solid var(--color-border, #e2e8f0)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.75rem', fontSize: '0.875rem' }}>
            ...
            <div><span style={{ color: '#64748b' }}>Assigned Tech:</span> {pred.assignedTechnicianName || 'Pending'}</div>
            <div style={{ gridColumn: '1 / -1' }}><span style={{ color: '#64748b' }}>Recommendation:</span> {pred.recommendation}</div>
          </div>
        </div>
      )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1rem' }}>
            {liveData.map((s: any) => (
              <div key={s.sensorId} style={{
                ...
                background: s.isAnomaly ? '#fef2f2' : '#f8fafc',
                border: `1px solid ${s.isAnomaly ? '#fca5a5' : '#e2e8f0'}`,
              }}>
                <div style={{ fontSize: '0.75rem', color: '#64748b', textTransform: 'uppercase' }}>{s.type}</div>
                <div style={{ fontSize: '1.25rem', fontWeight: 700, color: s.isAnomaly ? '#ef4444' : '#1e293b' }}>
      ...
      <div style={{ background: '#fff', borderRadius: 8, padding: '1.25rem', marginBottom: '1.5rem', border: '1px solid #e2e8f0' }}>
        ...
        {historyLoading ? (
          <div style={{ textAlign: 'center', padding: '2rem', color: '#888' }}>Loading chart...</div>
        ) : chartData.length > 0 ? (
          ...
          <div style={{ textAlign: 'center', padding: '2rem', color: '#888' }}>No history data available</div>
        )}
      </div>
      ...
      <div style={{ background: '#fff', borderRadius: 8, padding: '1.25rem', marginBottom: '1.5rem', border: '1px solid #e2e8f0' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.5rem', fontSize: '0.875rem' }}>
          ...
          <div><span style={{ color: '#64748b' }}>Firmware:</span> {m.firmwareVersion}</div>
          <div><span style={{ color: '#64748b' }}>Install Date:</span> {m.installDate}</div>
          <div><span style={{ color: '#64748b' }}>Next Maintenance:</span> {m.nextMaintenanceDate || 'N/A'}</div>
          <div><span style={{ color: '#64748b' }}>Tags:</span> {m.tags?.join(', ')}</div>
        </div>
      </div>
